    ...
    public Boolean closeSizeChart(WebDriver driver) {
        try {
            ...
            wait.until(ExpectedConditions
                    .invisibilityOfElementLocated(By.className("MuiDialog-paperScrollPaper")));
    ...
    public Boolean validateSizeChartContents(List<String> expectedTableHeaders,
        ...
        try {
            ...
            WebElement sizeChartParent =
                    driver.findElement(By.className("MuiDialog-paperScrollPaper"));
            WebElement tableElement = sizeChartParent.findElement(By.tagName("table"));
            List<WebElement> tableHeader =
                    tableElement.findElement(By.tagName("thead")).findElements(By.tagName("th"));
            ...
            for (int i = 0; i < expectedTableHeaders.size(); i++) {
                ...
                if (!expectedTableHeaders.get(i).equals(tempHeaderValue)) {
                    System.out.println("Failure in Header Comparison: Expected:  "
                            + expectedTableHeaders.get(i) + " Actual: " + tempHeaderValue);
                    status = false;
                }
            }
            ...
            for (int i = 0; i < expectedTableBody.size(); i++) {
                ...
                for (int j = 0; j < expectedTableBody.get(i).size(); j++) {
                    ...
                    if (!expectedTableBody.get(i).get(j).equals(tempHeaderValue)) {
                        System.out.println("Failure in Body Comparison: Expected:  "
                                + expectedTableBody.get(i).get(j) + " Actual: " + tempHeaderValue);
                        status = false;
                    }
                }
            }
